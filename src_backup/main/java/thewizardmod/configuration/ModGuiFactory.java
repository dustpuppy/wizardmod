...
public class ModGuiFactory implements IModGuiFactory 
{
	...
	public static class MBEConfigGui extends GuiConfig 
	{
		public MBEConfigGui(GuiScreen parentScreen) 
		{
			...
			super(parentScreen, getConfigElements(), TheWizardMod.MODID,
...
//			list.add(new DummyCategoryElement("miscCfg", "gui.mbe70_configuration.ctgy.other", CategoryEntryOther.class));
			list.add(new DummyCategoryElement("World Generation", "gui.configuration.ctgy.worldgen", CategoryEntryWorldgen.class));
			return list;
		}
/*		
...
*/
		public static class CategoryEntryWorldgen extends CategoryEntry
		{
			...
			{
        Configuration configuration = ModConfiguration.getConfig();
        ConfigElement cat_general = new ConfigElement(configuration.getCategory(ModConfiguration.CATEGORY_NAME_WORLDGEN));
        List<IConfigElement> propertiesOnThisScreen = cat_general.getChildElements();
        String windowTitle = configuration.toString();

        return new GuiConfig(this.owningScreen, propertiesOnThisScreen,
                             this.owningScreen.modID,
                             ModConfiguration.CATEGORY_NAME_WORLDGEN,
                             this.configElement.requiresWorldRestart() || this.owningScreen.allRequireWorldRestart,
                             this.configElement.requiresMcRestart() || this.owningScreen.allRequireMcRestart,
                             windowTitle);
        //this is a complicated object that specifies the category's gui screen, to better understand
