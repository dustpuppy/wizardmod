...
public class TileEntityDim extends TileEntity
{
	...
	public static void tele(EntityPlayer player)
	{
		...
		if ((player.getRidingEntity() == null) && ((player instanceof EntityPlayerMP)))
		{
			...
			if(player1.timeUntilPortal > 0)
			{
			...
			}else if(player1.dimension != thewizardmod.configuration.ModConfiguration.dimID){
				...
				if(prevX2 == 0.0 && prevY2 == 0.0 && prevZ2 == 0.0)
				{
					...
					mcServer.getPlayerList().transferPlayerToDimension(player1, thewizardmod.configuration.ModConfiguration.dimID, new MagicTeleporter(mcServer.worldServerForDimension(thewizardmod.configuration.ModConfiguration.dimID), dme2, 0, 0 ,0));
					...
					setDme22();
					setOverworldXYZ(player1.posX, player1.posY, player1.posZ);
					mcServer.getPlayerList().transferPlayerToDimension(player1, thewizardmod.configuration.ModConfiguration.dimID, new MagicTeleporter(mcServer.worldServerForDimension(thewizardmod.configuration.ModConfiguration.dimID), dme2, prevX2, prevY2, prevZ2));
				...
				setDme22();
				setTestXYZ(player1.posX, player1.posY, player1.posZ);
				mcServer.getPlayerList().transferPlayerToDimension(player1, 0, new MagicTeleporter(mcServer.worldServerForDimension(0), dme2, prevX, prevY, prevZ));
