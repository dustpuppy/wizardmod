 ...
 * for use in guis
 */
public class ContainerMagicInjector extends Container {

	// Stores the tile entity instance for later use
	private TileEntityMagicInjector tileInventoryMagicInjector;
	...
	public final int MAGICINJECTOR_SLOTS_COUNT = FUEL_SLOTS_COUNT + INPUT_SLOTS_COUNT + OUTPUT_SLOTS_COUNT;

	// slot index is the unique index for all slots in this container i.e. 0 - 35 for invPlayer then 36 - 49 for tileEntityMagicInjector
	...
	public ContainerMagicInjector(InventoryPlayer invPlayer, TileEntityMagicInjector tileInventoryMagicInjector) {
		this.tileInventoryMagicInjector = tileInventoryMagicInjector;

		final int SLOT_X_SPACING = 18;
		final int SLOT_Y_SPACING = 18;
		final int HOTBAR_XPOS = 8;
		final int HOTBAR_YPOS = 141;
		...
		final int PLAYER_INVENTORY_YPOS = 83;
		...
		final int FUEL_SLOTS_YPOS = 54;
		// Add the tile fuel slots
		for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
			int slotNumber = x + FIRST_FUEL_SLOT_NUMBER;
			addSlotToContainer(new SlotFuel(tileInventoryMagicInjector, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
		}
		...
		for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
			...
			addSlotToContainer(new SlotSmeltableInput(tileInventoryMagicInjector, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS+ SLOT_Y_SPACING * y));
		}

		final int OUTPUT_SLOTS_XPOS = 134;
		final int OUTPUT_SLOTS_YPOS = 18;
		// Add the tile output slots
		for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
			int slotNumber = y + FIRST_OUTPUT_SLOT_NUMBER;
			addSlotToContainer(new SlotOutput(tileInventoryMagicInjector, slotNumber, OUTPUT_SLOTS_XPOS, OUTPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
		}
	...
	{
		return tileInventoryMagicInjector.isUseableByPlayer(player);
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int sourceSlotIndex)
	{
		...
		if (sourceSlotIndex >= VANILLA_FIRST_SLOT_INDEX && sourceSlotIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
			...
			if (TileEntityMagicInjector.getSmeltingResultForItem(sourceStack) != null){
				if (!mergeItemStack(sourceStack, FIRST_INPUT_SLOT_INDEX, FIRST_INPUT_SLOT_INDEX + INPUT_SLOTS_COUNT, false)){
					return null;
				}
			}	else if (TileEntityMagicInjector.getItemBurnTime(sourceStack) > 0) {
		...
		} else if (sourceSlotIndex >= FIRST_FUEL_SLOT_INDEX && sourceSlotIndex < FIRST_FUEL_SLOT_INDEX + MAGICINJECTOR_SLOTS_COUNT) {
	...
	public void detectAndSendChanges() {
		...
		boolean fieldHasChanged [] = new boolean[tileInventoryMagicInjector.getFieldCount()];
		if (cachedFields == null) {
			cachedFields = new int[tileInventoryMagicInjector.getFieldCount()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i) {
			if (allFieldsHaveChanged || cachedFields[i] != tileInventoryMagicInjector.getField(i)) {
				cachedFields[i] = tileInventoryMagicInjector.getField(i);
    ...
    for (IContainerListener listener : this.listeners) {
			for (int fieldID = 0; fieldID < tileInventoryMagicInjector.getFieldCount(); ++fieldID) {
	...
	public void updateProgressBar(int id, int data) {
		tileInventoryMagicInjector.setField(id, data);
	}
	...
	public class SlotFuel extends Slot {
		...
		public boolean isItemValid(ItemStack stack) {
			return TileEntityMagicInjector.isItemValidForFuelSlot(stack);
		}
	}
	...
	public class SlotSmeltableInput extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return TileEntityMagicInjector.isItemValidForInputSlot(stack);
		}
	}
	...
	public class SlotOutput extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return TileEntityMagicInjector.isItemValidForOutputSlot(stack);
		}
	}
