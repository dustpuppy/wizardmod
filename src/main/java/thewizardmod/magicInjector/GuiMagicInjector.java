...
public class GuiMagicInjector extends GuiContainer {

	// This is the resource location for the background image
	private static final ResourceLocation texture = new ResourceLocation("thewizardmod", "textures/gui/magicInjector_bg.png");
	private TileEntityMagicInjector tileEntity;

	public GuiMagicInjector(InventoryPlayer invPlayer, TileEntityMagicInjector tileInventoryMagicInjector) {
		super(new ContainerMagicInjector(invPlayer, tileInventoryMagicInjector));

		// Set the width and height of the gui
		xSize = 176;
		ySize = 207;

		this.tileEntity = tileInventoryMagicInjector;
	}

	// some [x,y] coordinates of graphical elements
	final int COOK_BAR_XPOS = 49;
	final int COOK_BAR_YPOS = 18;
	final int COOK_BAR_ICON_U = 1;   // texture position of arrow icon
	...
	int FLAME_XPOS = 81;
	int FLAME_YPOS = 37;
	int FLAME_ICON_U = 176;   // texture position of flame icon
	int FLAME_ICON_V = 0;
	int FLAME_WIDTH = 14;
	int FLAME_HEIGHT = 14;
	int FLAME_X_SPACING = 18;
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		for (int i = 0; i < tileEntity.FUEL_SLOTS_COUNT; ++i) {
			if (isInRect(guiLeft + FLAME_XPOS + FLAME_X_SPACING * i, guiTop + FLAME_YPOS, FLAME_WIDTH, FLAME_HEIGHT, mouseX, mouseY)) {
				hoveringText.add("Time left:");
				// Regarding on burn time of shadow dust we calc the amount in fuel
				int lm = 10*tileEntity.secondsOfFuelRemaining(i);
				hoveringText.add(lm + "%");
			}
		}
