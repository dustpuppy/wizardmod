...
*/
	public static int chance_of_cherry_trees;
	public static int num_of_cherry_trees;
	
	public static boolean GenerateNether;
	public static boolean GenerateEnd;
	
	public static int dimID;
	
	
	
	public static final String CATEGORY_NAME_WORLDGEN = "category_worldgen";
	...
	public static void preInit() 
	{
		 ...
		 */
		File configFile = new File(Loader.instance().getConfigDir(), "TheWizardMod.cfg");
...
*/
		...
		final int CHANCE_CHERRY_TREES_MIN_VALUE = 1;
		final int CHANCE_CHERRY_TREES_MAX_VALUE = 100;
		final int CHANCE_CHERRY_TREES_DEFAULT_VALUE = 10;
		Property propChanceOfCherryTrees = config.get(CATEGORY_NAME_WORLDGEN, "chance_of_cherry_trees", CHANCE_CHERRY_TREES_DEFAULT_VALUE,
				"Chance to get Cherry Trees generated in Chunk", CHANCE_CHERRY_TREES_MIN_VALUE, CHANCE_CHERRY_TREES_MAX_VALUE);
		propChanceOfCherryTrees.setLanguageKey("gui.configuration.chance_of_cherry_trees");

		final int NUM_CHERRY_TREES_MIN_VALUE = 1;
		final int NUM_CHERRY_TREES_MAX_VALUE = 20;
		final int NUM_CHERRY_TREES_DEFAULT_VALUE = 2;
		Property propNumOfCherryTrees = config.get(CATEGORY_NAME_WORLDGEN, "num_of_cherry_trees", NUM_CHERRY_TREES_DEFAULT_VALUE,
				"Amount of Cherry Trees generated per Chunk", NUM_CHERRY_TREES_MIN_VALUE, NUM_CHERRY_TREES_MAX_VALUE);
		propNumOfCherryTrees.setLanguageKey("gui.configuration.num_of_cherry_trees");

		final boolean GENERATE_IN_NETHER_DEFAULT_VALUE = false;
		Property propGenerateInNether = config.get(CATEGORY_NAME_WORLDGEN, "generateOresInNether", GENERATE_IN_NETHER_DEFAULT_VALUE);
		propGenerateInNether.setComment("If you want Ores generated in Nether set true");
		propGenerateInNether.setLanguageKey("gui.configuration.generateInNether");
		
		final boolean GENERATE_IN_END_DEFAULT_VALUE = false;
		Property propGenerateInEnd = config.get(CATEGORY_NAME_WORLDGEN, "generateOresInEnd", GENERATE_IN_END_DEFAULT_VALUE);
		propGenerateInEnd.setComment("If you want Ores generated in End set true");
		propGenerateInEnd.setLanguageKey("gui.configuration.generateInEnd");
		
		final int DIM_MIN_VALUE = 3;
		final int DIM_MAX_VALUE = 250;
		final int DIM_DEFAULT_VALUE = 3;
		Property propNumDimId = config.get(CATEGORY_NAME_WORLDGEN, "dimension_id", DIM_DEFAULT_VALUE,
				"ID for the Magic Dimension", DIM_MIN_VALUE, DIM_MAX_VALUE);
		propNumDimId.setLanguageKey("gui.configuration.dimension_id").setRequiresMcRestart(true);
...
*/
		...
		propOrderWorldgen.add(propChanceOfCherryTrees.getName());
		propOrderWorldgen.add(propNumOfCherryTrees.getName());
		propOrderWorldgen.add(propGenerateInNether.getName());
		propOrderWorldgen.add(propGenerateInEnd.getName());
		propOrderWorldgen.add(propNumDimId.getName());
		config.setCategoryPropertyOrder(CATEGORY_NAME_WORLDGEN, propOrderWorldgen);
...
*/			
			chance_of_cherry_trees = propChanceOfCherryTrees.getInt(CHANCE_CHERRY_TREES_DEFAULT_VALUE);
			if(chance_of_cherry_trees > CHANCE_CHERRY_TREES_MAX_VALUE || chance_of_cherry_trees < CHANCE_CHERRY_TREES_MIN_VALUE)
			{
				chance_of_cherry_trees = CHANCE_CHERRY_TREES_DEFAULT_VALUE;
			}

			num_of_cherry_trees = propNumOfCherryTrees.getInt(NUM_CHERRY_TREES_DEFAULT_VALUE);
			if(num_of_cherry_trees > NUM_CHERRY_TREES_MAX_VALUE || num_of_cherry_trees < NUM_CHERRY_TREES_MIN_VALUE)
			{
				num_of_cherry_trees = NUM_CHERRY_TREES_DEFAULT_VALUE;
			}

			GenerateNether = propGenerateInNether.getBoolean(GENERATE_IN_NETHER_DEFAULT_VALUE);
			GenerateEnd = propGenerateInEnd.getBoolean(GENERATE_IN_END_DEFAULT_VALUE);

			dimID = propNumDimId.getInt(DIM_DEFAULT_VALUE);
			if(dimID > DIM_MAX_VALUE || dimID < DIM_MIN_VALUE)
			{
				dimID = DIM_DEFAULT_VALUE;
			}
...
*/
		propChanceOfCherryTrees.set(chance_of_cherry_trees);
		propNumOfCherryTrees.set(num_of_cherry_trees);
		propGenerateInNether.set(GenerateNether);
		propGenerateInEnd.set(GenerateEnd);
		propNumDimId.set(dimID);
	...
	public static class ConfigEventHandler 
	{
		...
		{
			if (TheWizardMod.MODID.equals(event.getModID()) && !event.isWorldRunning()) 
			{
//				if (event.getConfigID().equals(CATEGORY_NAME_GENERAL) || event.getConfigID().equals(CATEGORY_NAME_OTHER) || event.getConfigID().equals(CATEGORY_NAME_WORLDGEN)) 
				if (event.getConfigID().equals(CATEGORY_NAME_WORLDGEN)) 
				{
					syncFromGUI();
				}
			}
